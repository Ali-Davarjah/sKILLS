# ارزیابی-فروشنده-پگاه

اسکیل ارزیابی فروشنده پگاه، در فرمت استاندارد `SKILL.md`. مستقل است: این پوشه را
هرجا بگذاری کار می‌کند.

```
SKILL.md                      دستورالعمل — چیزی که مدل هر بار می‌خواند
rules.json                    آستانه‌ها، گام‌ها و ضریب‌ها (داده، نه کد)
scripts/score.py              نمره‌دهی
references/queries.md         کوئری‌های آماده و تست‌شده روی pakhsh
references/metrics.md         کدام جدول، کدام ستون، و کدام جدول‌ها مرده‌اند
references/reporting.md       شکل گزارش
examples/sample-input.json    نمونه ورودی کامل
```

## نگاشت داده

روی دیتابیس `pakhsh` (SQL Server) نوشته و تست شده:

| معیار | منبع |
|---|---|
| ۱ واحد مغازه‌روز با ضریب | `Sales.AmarForosh_Arshive` + ضریب از `Sales.ZaribNoeMoshtary` |
| ۲ درصد ویزیت مثبت | `Sales.VisitForoshandeh_Arshiv` — رویدادِ ویزیت |
| ۳ میانگین سطر هر فاکتور | `Sales.DarkhastFaktorSatr` — `COUNT(*)` |
| ۴ درصد مشتری خرید کرده | `VisitForoshandeh_Arshiv` — مشتریِ متمایز (فقط ماهانه) |
| ۵ درصد مرجوعی مبنادار | `Sales.ElamMarjoee(Satr)` با `MasoleiatElat = 1` — کالا ÷ کالا |
| ۶ درصد تحقق کل هدف | `Sales.HadafForoshRoozanehNew` (فقط ماهانه، سقف ۱۲۰٪ هر گروه) |
| تقویم | `Global.Taghvim` (`CodeNoeTatili IS NULL`) |
| گروه محصول | `Global.Goroh` با `ccGorohLink = 560` |
| لاین فروش | `Global.SazmanForosh` |
| نام فروشنده | `Global.Afrad` از `ccAfradForoshandeh` |

**دوره‌ی روزانه چهار معیار دارد، دوره‌ی «تا روز» شش تا.**

چهار تله که وقت زیادی می‌گیرند اگر از قبل ندانی:

- `Sales.AmalkardRozanehForosh` و `Sales.HadafForosh` از **۲۰۱۸** به‌روز
  نمی‌شوند و اسمشان دقیقاً همین کار است.
- `Sales.ElamMarjoee` **اعلامِ** مرجوعی است نه مرجوعیِ ثبت‌شده — ۱۰ تا ۲۵ برابر
  کمتر.
- بدون فیلتر `ccNoeForoshandeh = 1`، سرپرست و رییس مرکز صدر جدول را می‌گیرند.
- معیار ۶ اگر میانگینِ گروه‌ها گرفته شود تا ۴۲۳٪ بالا می‌رود؛ نسبت کل درست است.

## ایمپورت

در بخش Skills، «افزودن از URL» و آدرس این پوشه در گیت‌هاب:

```
https://github.com/<owner>/<repo>/tree/main/ارزیابی-فروشنده-پگاه
```

کل پوشه با هم می‌آید. چون اسکریپت اجرایی دارد، اسکیل **خاموش** ایمپورت می‌شود؛
بعد از اینکه یک نفر `scripts/score.py` را خواند، روشنش کنید. تغییرات این فایل‌ها
تا ایمپورت دوباره به محصول نمی‌رسد.

## اجرای مستقیم

```bash
python scripts/score.py examples/sample-input.json --period cumulative
python scripts/score.py examples/sample-input.json --period daily --summary
python scripts/score.py - --period daily --json < metrics.json
```

پایتون ۳.۹ به بالا، بدون هیچ وابستگی بیرونی.

## تغییر قاعده‌ها

همه عددهای ارزیابی در `rules.json` است — مبنا، گام، نمره هر گام، ضریب نوع مشتری،
و خط حذف مرجوعی. برای تغییر آستانه، اسکریپت را دست نزنید. `version` را هم عوض
کنید تا معلوم باشد گزارش‌های قدیمی با کدام قاعده درآمده‌اند.

## تصمیم‌هایی که در قاعده‌ها کدگذاری شده

- نمره‌دهی **پیوسته** است: ویزیت مثبت ۴۳.۴٪ با مبنای ۴۰٪ و ۰.۵ نمره در هر ۱٪
  می‌شود ۱.۷ نمره. برای حالت پله‌ای، `"stepping": "stepped"` در `rules.json`.
- معیار ۶ **نسبت جمعِ فروش به جمعِ هدف** است، نه میانگینِ درصدِ گروه‌ها — و هر
  گروه سقف **۱۲۰٪** دارد که پیش از جمع زدن اعمال می‌شود (`cap_pct`).
- باندهای وضعیت: ۱۸ به بالا عالی، ۱۶ تا ۱۸ خوب، ۱۲ تا ۱۶ متوسط، و کمتر از ۱۲
  ضعیف.
- مرجوعی بالای ۲٪ در ارزیابی «تا روز» نمره کل را صفر می‌کند و آن فروشنده ته
  جدول می‌نشیند. نمره خام در `--json` با کلید `raw_total` می‌ماند.

## آنچه هنوز باید با مدیر فروش پگاه چک شود

- **معیار ۴ ارزیابی روز** در برگه اصلی قاعده نداشت؛ قاعده ماهانه (۸۰٪، هر ۱٪
  معادل ۰.۵ نمره) جایش گذاشته شده و در گزارش پاورقی می‌گیرد.
- **کد ۳۵۳ «شبه عمده»** ضریبی نمی‌گیرد. همین‌طور ۳۴۹ تعاونی ویژه، ۳۵۱ و ۳۵۲
  نماینده، ۶۰۷ تعاونی کارکنان.
- **هدف روزانه** از تقسیم هدف ماهانه بر روزهای کاری درمی‌آید.

## پیوند با بقیه‌ی اسکیل‌ها

این اسکیل **قاعده‌ی کسب‌وکاری** است و از سه اسکیما می‌خواند. برای هر سه
اسکیلِ نقشه هست:

| چه می‌خواهی | اسکیل |
|---|---|
| هر جدول یا ستونِ دیگرِ فروش، مسیر اتصال، تله‌ها | `تسلط-فروش-پگاه` |
| تقویم شمسی، گروه کالا (`ccGorohLink=560`)، `Afrad`، لاین فروش | `تسلط-مشترک-پگاه` |
| علتِ مرجوعی و `MasoleiatElat`، کالا | `تسلط-انبار-پگاه` |
| رتبه و درجه‌ی خودِ مشتری‌ها | `رتبه-بندی-مشتریان-پگاه` |

`ccAfradForoshandeh` جدولی به این نام ندارد — `Global.Afrad` است در نقشِ
فروشنده. سؤالِ بازِ «شخص یا مسیر؟» دقیقاً روی همین پل می‌چرخد.

**نامِ دیتابیس:** کوئری‌های این اسکیل روی اتصالی به نامِ `pakhsh` تست
شده‌اند؛ نقشه‌های اسکیما کاتالوگ را `PegahAI` می‌نامند. نامِ اسکیما و جدول
در هر دو یکی است.

نقشه‌ی کامل خانواده، ماتریسِ ارجاع و فهرستِ ارجاع‌های حل‌نشده در
[../SKILLS.md](../SKILLS.md). عددها را `crosslink.py` می‌سازد؛ دستی
به‌روزشان نکن.
